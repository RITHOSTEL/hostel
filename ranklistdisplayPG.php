	...
	?>		 
		...
		<table>
			<tr>
				...
				<td>
				<form name="frm2" method ="POST" action="ranklistgenerateMH.php">
					&nbsp &nbsp &nbsp<input type="submit" name="subrankgenerateMH" value="Rank list for MH" >
				</form>
				</td>
				<td>
				<form name="frm3" method ="POST" action="ranklistgeneratePG.php">
...
LEFT JOIN class_details cd ON cd.classid=cc.classid where hsr.admn_status in ('ranked') AND  cd.courseid in('MTECH','MCA') AND cd.semid in(1,2) and sd.gender='M' order by hsr.final_rank asc ";
